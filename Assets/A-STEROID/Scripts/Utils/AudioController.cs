...
public class AudioController : MonoBehaviour
{
	...
	public void PlayAtPosition(AudioClip clip, AudioMixerGroup mixerGroup, Vector3 position, float volume = 1)
	{
		var audio = AudioSourcePool.instance.NewAudioSource(clip, mixerGroup, volume);

		audio.transform.position = position;
		audio.Play();

		StartCoroutine(FreeAudioSource(audio));
	}

	IEnumerator FreeAudioSource(AudioSource audio)
	{
		yield return new WaitForSeconds(audio.clip.length);

		AudioSourcePool.instance.FreeAudioSource(audio);
	}
	...
	public void PlayDamageAtPosition(Vector3 position, float volume = 1) { PlayAtPosition(damageClip, damageMixerGroup, position, volume); }
	public void PlayExplosionAtPosition(Vector3 position, float volume = 1) { PlayAtPosition(explosionClip, explosionMixerGroup, position, volume); }
	public void PlayLaserAtPosition(Vector3 position, float volume = 1) { PlayAtPosition(laserClip, laserMixerGroup, position, volume); }
